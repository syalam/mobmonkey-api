...
public class SearchResource extends ResourceHelper {
	...
	public Response findLocationsInJSON(Location loc) {
		...
		if(loc.getLocality() != null && loc.getRegion() != null && loc.getPostcode() != null){
			//TODO validate postcode with regex, make sure locality and region are sane
			List<Location> locations = SearchHelper.getLocationsByAddress(loc);
			return Response.ok().entity(locations).build();
			
		}
		return Response.status(500).entity(new Status("Failure", "You need to specify either (lat & long) OR (address, locality, region & zip)", "")).build();
	}
